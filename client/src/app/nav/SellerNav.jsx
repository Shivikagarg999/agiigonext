...
export default function Nav() {
  ...
  const [isSeller, setIsSeller] = useState(true); // Assume user is a seller (Get this from context/cookie in a real app)
  ...
  const handleLogout = async () => {
    try {
      ...
      if (response.ok) {
        Cookies.remove("user"); // Clear authentication cookie
  ...
  return (
    ...
    <div className="container mx-auto flex justify-between items-center px-6">
      ...
      <div className="hidden lg:flex items-center space-x-4">
        <a href="/profile" className="text-[#EB8426] hover:text-black flex items-center space-x-1">
          <User className="h-6 w-6" />
          <span>Profile</span>
        </a>
        {isSeller && <a href="/pages/addProducts" className="text-[#EB8426] font-semibold">Add Product</a>}
        {isSeller ? (
    ...
    {isOpen && (
      <div className="lg:hidden bg-white shadow-md py-4 px-6">
        <ul className="flex flex-col space-y-4 text-lg font-semibold">
          ...
          {isSeller && <li><a href="/my-products" className="text-gray-700 hover:text-black">My Products</a></li>}
          <li>
            <a href="/seller-profile" className="text-[#EB8426] hover:text-black flex items-center space-x-1">
              <User className="h-6 w-6" />
              <span>Profile</span>
            </a>
          </li>
